 ...
 * shop-UNIFORM calendar config, keyed by shop id â€” same pattern as
 ...
 * same list â€” unlike appointments/blocked_time, which are strict-barber
 ...
 *
 * Service-role client + explicit `shop_id` scope â€” service-role bypasses RLS,
 ...
 * busts that shop's entry â€” a global tag would invalidate every tenant's
 ...
 * explicitly â€” `revalidatePublicShopSurfaces(alias)` does so.
 */
 ...
 * key includes the alias and the tag is `shop-alias:${alias}` â€” busted by
 ...
 * (the route 404s). Matches the route's original `.eq('alias', â€¦).limit(1)`.
 */
 ...
 * Tag `barber-settings:${shopId}` â€” busted by `revalidateShopConfig` from the
...
/**
 * Confirmed + bookable barbers (id + sort_order, sort-order ascending) â€” the
 * slots route's "pick first for `any`" source AND its explicit-barber
 * validation list (membership = bookable in this shop). Tag
 * `bookable-barbers:${shopId}` â€” busted by `revalidateShopConfig` from the
