...
- **Email branding (reminder/birthday) DIFFÉRÉ** : première tentative rejetée en review (n'unifiait que 2 des 7 templates + ajoutait un footer « Powered by Küa » absent des 5 autres = nouvelle incohérence ; layout sans props white-label `emailLogoUrl`/`emailAccentColor`). À refaire proprement : un layout brandé partagé pour LES 7 templates, avec support white-label par shop.
