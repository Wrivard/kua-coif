...
## Wave 2/3 — batch 1 : tests de câblage + gestion des catégories de services (2026-06-05)

- **Garde-fou i18n + tests de câblage** : ajout de `tests/i18n-parity.test.ts` (assert que les jeux de clés feuilles de `messages/en.json` et `fr.json` sont identiques), de tests `withAction` (toutes les portes : UNAUTHENTICATED/NO_SHOP/FORBIDDEN/INVALID_INPUT/UNEXPECTED + succès) et `mapIntentStatus` (Stripe). Dérive i18n pré-existante corrigée : `auth.login.noAccount` + `signupLink` manquaient côté `en` → ajoutés (NB : ces 2 clés sont actuellement **non câblées** — pas de page signup ; conservées pour la parité, à câbler ou supprimer plus tard). `vitest.config.ts` exclut désormais `**/.claude/**` (les worktrees isolés des agents Workflow sont des copies complètes du repo → sans ça `npm test` re-roule chaque test N fois).
- **Gestion des catégories de services** (`/services`, bouton « Categories », manager-gated) : CRUD via server actions `withAction` + `logAuditAction` + `revalidatePath`. **Suppression gardée** : renvoie `CONFLICT` si des services référencent encore la catégorie (plus sûr que `deleteBrand`/`deleteCategory` côté produits qui n'ont PAS ce garde — incohérence à corriger plus tard). Modal réutilise Modal + ConfirmDialog.
- **Email branding (reminder/birthday) DIFFÉRÉ** : première tentative rejetée en review (n'unifiait que 2 des 7 templates + ajoutait un footer « Powered by Küa » absent des 5 autres = nouvelle incohérence ; layout sans props white-label `emailLogoUrl`/`emailAccentColor`). À refaire proprement : un layout brandé partagé pour LES 7 templates, avec support white-label par shop. **→ FAIT en batch 2b (voir ci-dessous).**

## Wave 1/UI + Wave 3 — batch 2b : vue Week + branding email unifié (2026-06-05)

- **Vue Week du calendrier** : 3e vue via le toggle existant (SxS/Week/List). `page.tsx` fetch la semaine (Lun..Dim contenant la date) UNIQUEMENT quand `?view=week` (le fetch jour reste inchangé pour SxS/List → pas de coût DB ailleurs). Grille 7 colonnes, réutilise le drawer déjà monté + `statusToColor` (exporté). Nits connus non-bloquants : le badge count de l'onglet Week affiche 0 hors vue week (cosmétique) ; `?view=` se désync sur List→SxS (comportement pré-existant, pas une régression).
- **Branding email unifié (refait correctement)** : un layout partagé `lib/email/templates/branded-layout.tsx` appliqué aux **7** templates transactionnels (même header + même footer partout), avec **white-label par shop** (`logoUrl`/`accentColor`, fallback wordmark Küa + `#8b5cf6`) — préserve le white-label existant de confirmation + waitlist. Convention `copy(locale)` inline conservée (PAS de next-intl dans les emails). Net −205 lignes (déduplication des helpers Row/Label/palette).

## Wave 2/3 — batch 3 : réordonnancement Services + tests rate-limit (2026-06-05)

- **Services drag-to-reorder** : @dnd-kit/sortable réel sur la liste Services, persiste `sort_order` via `reorderServices` (withAction manager + audit + revalidate). Remplace le rendu DataTable read-only par un DndContext/SortableContext dédié (table desktop + cards mobile), poignée de drag dédiée (les boutons edit/toggle/delete restent cliquables) ; le modal de catégories récent est préservé. Nits non-bloquants : pas de KeyboardSensor (a11y clavier — convention pré-existante du calendrier) ; persistance N updates non-atomique (auto-réparée au prochain reorder/reload) ; duplication de layout vs DataTable (justifiée : le prop `reorderable` de DataTable était décoratif).
- **Rate-limit distribué Upstash : DÉJÀ FAIT en Phase 21** (commit `207ba64` — slidingWindow + fallback in-memory). L'agent a découvert l'implémentation existante et a seulement AJOUTÉ un test du chemin fallback (le titre de commit `feat(security): distributed Upstash rate-limit` sur-déclare ; c'est en réalité un `test`). **Leçon : la mémoire roadmap sous-estime l'avancement réel — toujours vérifier l'état du code avant d'implémenter** (les agents Workflow le font via leur lecture préalable).

## Grill money + audit profond money/auth (2026-06-13)

> Session `/grill-with-docs` (arbre money) + audit profond des 2 verticales à haut risque (money, auth). Glossaire de domaine extrait dans `CONTEXT.md` (nouveau fichier). Décisions arbitrées avec l'owner.

- **Revenue = NET des remboursements ; pourboires en ligne SÉPARÉS ; JAMAIS dans la base de commission.** `lib/business/finances.ts` (`excludeRefunded`/`netRevenue`) est la définition unique, partagée par `/finances` et `/finances/today` ; la base de commission par barbier réutilise la même règle (vérifié cohérent au code). Nit : le KPI s'appelle encore « Revenu brut / Gross revenue » alors qu'il est net — libellé à renommer (backlog).
- **Remboursements = tout-ou-rien (modèle `payment_status` binaire).** Pas de statut `partially_refunded`. Le webhook `charge.refunded` ne marque `'refunded'` QUE sur un remboursement TOTAL (`amount_refunded >= amount`) ; un partiel (action manuelle dashboard Stripe, rare) est loggué Sentry `stage:'partial-refund'` pour réconciliation manuelle, jamais appliqué — sinon il effacerait 100 % du revenu + la commission du barbier. Le suivi du montant remboursé (colonne `refunded_amount_cents`) est différé jusqu'à ce qu'un salon fasse réellement des partiels.
- **Dépôts confisqués sur no-show = ligne SÉPARÉE, jamais fondue dans le revenu de service.** `forfeitedDeposits()` (`status='no_show'` + `payment_status='paid'` + `deposit_amount_cents>0`) surfacé en KPI sur `/finances` et `/today`. C'est un no-show fee (nature ≠ vente, traitement TPS/TVQ potentiellement distinct).
- **Loyalty NON clawé-back au remboursement** (chemins booking ET admin, AM-1) : un RDV complété qui a accru de la fidélité / émis une récompense puis est remboursé GARDE la progression. Accepté comme coût marketing borné (le claw-back d'une récompense peut-être déjà dépensée est complexe). Différé.
- **`deposit_amount_cents` est mal nommé** : snapshot du MONTANT CHARGÉ EN LIGNE (base dépôt + pourboire en ligne), pas un pur dépôt. Pas renommé (colonne DB, coûteux) — documenté dans `CONTEXT.md`.
- **Booking (surface publique anonyme) durci** : exige un PaymentIntent quand le shop peut + veut charger en ligne (H1 — sinon un POST omettant le PI réservait impayé) ; ne rembourse pas un PI qui adosse déjà un RDV (H2 — double-submit) ; overlap `23P01` → CONFLICT (L1) ; échec de débit loyalty rendu durablement réconciliable via `logDurableAudit`, sans migration (M2).
- **Cache memberships/rôle busté sur mutation** (AUTHZ-R1) : `revalidateTag('memberships')` ajouté aux 6 mutations `shop_members` (la révocation/rétrogradation ne lag plus ≤60s). `is_kua_admin` posé en DB only → aucun site app à buster.
- **Audit profond = borné aux 2 verticales à haut risque (money + auth).** 3 surfaces money (booking/admin/self-service) + auth/super-admin ratissées : seul le **booking public** avait des HIGH réels (corrigés) ; admin + self-service + auth = solides. Le reste (calendrier/clients/produits/marketing/UI) reste couvert par l'audit large antérieur — deep-audit non poursuivi (rendement marginal).
