...
/**
 * Plan 041 (PERF-09) — scoped i18n provider for the reschedule token surface
 ...
 */
export default async function RescheduleSegmentLayout(props: {
  children: ReactNode;
  params: Promise<{ locale: string }>;
}) {
  const { locale } = await props.params;
  setRequestLocale(locale);
  const messages = pickMessages(await getMessages(), ['pages.reschedule', 'a11y']);
  return (
