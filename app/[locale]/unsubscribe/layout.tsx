...
/**
 * Plan 041 (PERF-09) — scoped i18n provider for the unsubscribe token surface
 ...
 */
export default async function UnsubscribeSegmentLayout(props: {
  children: ReactNode;
  params: Promise<{ locale: string }>;
}) {
  const { locale } = await props.params;
  setRequestLocale(locale);
  const messages = pickMessages(await getMessages(), ['pages.unsubscribe', 'a11y']);
  return (
