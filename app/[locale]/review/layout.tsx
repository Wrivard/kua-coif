...
/**
 * Plan 041 (PERF-09) — scoped i18n provider for the review token surface
 ...
 */
export default async function ReviewSegmentLayout(props: {
  children: ReactNode;
  params: Promise<{ locale: string }>;
}) {
  const { locale } = await props.params;
  setRequestLocale(locale);
  const messages = pickMessages(await getMessages(), ['pages.review', 'a11y']);
  return (
